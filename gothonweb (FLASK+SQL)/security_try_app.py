...
app.config['FLASK_ADMIN_SWATCH'] = 'readable'    #defining the admin css theme
...
#------------------------
# Adding & managing the admin panel and the authentication
admin = Admin(app, name='Gothonweb-admin', template_mode='bootstrap3')

class GothonModelView(ModelView):
...
admin.add_view(GothonModelView(User, db.session))   
...
def login():
    print(f"user status no: {new_user}")
    users = User.query.all()
...
# def admin():
#     users = User.query.all()
#     return render_template(url_for('admin', users=users))
